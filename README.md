...
- **0.4.0** — adaptive replanning. Nodes with `adaptive: true` can signal `[REPLAN: reason]` in their output, triggering AI-driven DAG mutation mid-run. Two-layer governance: approval modes (`auto`/`confirm`/`log`) control autonomy boundary, mandatory `justification` field anchors replans to the original goal.
